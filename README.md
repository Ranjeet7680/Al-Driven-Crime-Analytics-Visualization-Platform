...
[![Zero Backend](https://img.shields.io/badge/Architecture-100%25%20Zero--Backend%20SPA-06b6d4?style=for-the-badge)](index.html)
[![WCAG AA](https://img.shields.io/badge/Accessibility-WCAG%20AA%20Compliant-10b981?style=for-the-badge)](index.html)
...
```mermaid
flowchart TD
    subgraph DataSources["📁 Data Sources (Karnataka Police 2025)"]
        D1["202,533 Total Crime Records"]
        D2["37 Administrative Jurisdictions"]
        D3["76+ IPC/BNS & SLL Crime Heads"]
        D4["Temporal & Seasonal Metrics"]
    end

    subgraph StateEngine["⚙️ State & Signal Engine (data.js & localStorage)"]
        S1["Geospatial Boundary & Coordinate Mapping"]
...
sequenceDiagram
    ...
    participant Model as AI Forecast Engine
    participant Twin as Digital Twin Simulator
    participant Audio as Web Audio Synthesizer

    User->>Finder: Press Ctrl+K -> Search District ("Tumakuru")
    Finder->>UI: Open District Intelligence Deep-Dive Modal
    Audio-->>User: Trigger Web Audio 'pop' Chime (350Hz -> 1100Hz)
    UI->>Model: Query Forecast for District & Crime Category
    Model-->>UI: Return Risk Score (76/100), AI Directives, Crime Head Breakdown
    User->>Twin: Adjust Patrol Intensity (+40%) & CCTV Density (+25%)
    Twin-->>UI: Real-Time Incident Reduction Projection (-18.4%)
    User->>UI: Click "Export Executive Report"
    UI-->>User: Generate JSON File & Formatted Print Preview
```
